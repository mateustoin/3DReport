...
actual fun periodStartEpochMillis(preset: PeriodPreset): Long? {
    ...
    val startDate = when (preset) {
        ...
        PeriodPreset.LAST_MONTH -> today.withDayOfMonth(1).minusMonths(1)
        PeriodPreset.THIS_YEAR -> today.withDayOfYear(1)
    }
    return startDate.atStartOfDay(zone).toInstant().toEpochMilli()
}

actual fun periodEndEpochMillis(preset: PeriodPreset): Long? {
    if (preset != PeriodPreset.LAST_MONTH) return null
    val zone = ZoneId.systemDefault()
